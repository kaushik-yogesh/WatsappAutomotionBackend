...
// ✅ FIXED: Embedded Signup Callback
exports.embeddedSignupCallback = async (req, res, next) => {
    try {
        ...
        // ✅ Directly use accessToken (NO oauth call)
        const wabaRes = await axios.get(`${META_API_BASE}/me/businesses`, {
            params: {
        ...
        const metaErr = err.response?.data?.error;
        logger.error('Embedded signup error:', metaErr || err.message);
        const msg = metaErr?.message || 'Failed to fetch WhatsApp data.';
        next(new AppError(msg, 502));
    }
};
